...
class GameView(arcade.Window):
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        if self.build_mode:
            collisions = arcade.check_for_collision_with_list(
                self.collision_sprite, self.grid.sprite_list
            )
    ...
    def build_thing(self, grid_site: tuple[int, int]):
        ...
        if self.grid.grid_logic.check_build_on_tiles(
            grid_site, thing_to_build.grid_size
        ):
            print(f"empty tile found at: {grid_site}")
            self.grid.grid_logic.reassign_tiles(
                grid_site, thing_to_build.grid_size, LandType.BUILDING
            )
            thing = Building(
                char_sheet_spec=thing_to_build.character_sheet, scale=SCALE
            )
